...
# MAIN SETTINGS:
x_sensitivity, y_sensitivity = 2, 2
walking_speed = 3
framerate = 120
...
def polygonconnect(polyhedron):
    proj_vertices = [clipspace(vertex) for vertex in polyhedron.vertices]
    ...
    for face in polyhedron.faces:
        ...
        # if len(good) > 0:
            for i, coord in enumerate(map):
        #         if i in bad:
        #             coord = clip(good[0], coord, 0)
                map[i] = raster(coord)
            pg.draw.polygon(screen, (255, 255, 255), map, 3)

def lineconnect(line):
    [clipspace(vertex) for vertex in line.vertices]
    pg.draw.lines()
...
while True:
    ...
    altaz[0] += mouse_movement[1] * y_sensitivity * m.pi / 3600
    if altaz[0] > m.pi / 2:
        altaz[0] = m.pi / 2
    elif altaz[0] < -m.pi / 2:
        altaz[0] = -m.pi / 2

    altaz[1] += mouse_movement[0] * x_sensitivity * m.pi / 3600
